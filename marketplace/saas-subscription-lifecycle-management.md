...
Azure Market'dan SaaS teklifi satın aldığınız zaman, Azure portal'de hizmet olarak yazılım (SaaS) aboneliği oluşturur (ayrıntılar için [bkz.](purchase-saas-offer-in-azure-portal.md)Azure portal). Portalda bir SaaS kaynağı bulmak için:
...
- **Tüm kaynaklar** – Erişiminiz olan tüm abonelikler altında oluşturulan tüm kaynakları listeler. Belirli bir aboneliği bulmak için filtreyi kullanın (SaaS > yazın) veya listede arama.
...
Azure portal Şubat 2021'den önce bir SaaS aboneliği satın aldıysanız, bu abonelik kiracınız altında oluşturulmuş ve SaaS Klasik'in altında yer almaktadır. Bu listede Microsoft AppSource satın almalar vardır.
...
- Azure aboneliğinden ve Kaynak gruplarından devralınan ilkeler ve izinlerle SaaS kaynağınıza erişimi ve denetimi iyileştirildi (tam RBAC denetimi)
- SaaS'nin yaşam döngüsü, aktarımlar ve silmeler gibi eylemler için Azure aboneliğinin yaşam döngüsüne bağlanır
- SaaS kaynaklarıyla daha kolay bulunabilirlik, genel arama
...
Satın alma sonrasında gerçekleştir eylemler gerçekleştirin; Bazılarında Sahip veya Katkıda Bulunan izinleri gerekir:

### <a name="change-plans"></a>Değişiklik planları

Abone olduğunuz bir planda yaptığınız değişiklikler hemen etkili olur. Faturalama, geçerli planın faturalama dönemine göre provokasyona göre olacak şekilde olacak. Planları değiştirmek için **Sahip veya Katkıda** Bulunan **izinleri** gerekir.
...
Lisans tabanlı planlar için satın alma işlemi sırasında tanımlandığı kullanıcı sayısını ekleyebilir veya azaltabilirsiniz. Abone olduğunuz kullanıcı sayısını değiştirmek hemen geçerli olur ve faturalama geçerli planın faturalama dönemine göre provokasyona tabi olur. Yer değişiklikleri yalnızca yayımcı tarafından tanımlandığı gibi en düşük ve en yüksek yer aralığı içinde mümkündür. Bazı durumlarda, yer değiştirmeden önce bir planı değiştirmelisiniz ve bunun tersi de geçerlidir.
...
İptal etme işlemi, bu SaaS aboneliğinin bir parçası olarak satın aldığınız yazılıma erişiminizi kaldırır. Para iadeleri para iadesi ilkesine göre işlenir; Ayrıntılar için bkz. [Microsoft AppSource ve Azure Market.](refund-policies.md)
