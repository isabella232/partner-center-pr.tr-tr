...
> [!Important]
> Anlaşma türü filtresi, tüm veriler için seçilen **ortak satış** ve **iş ortağı** türü ile önceden uygulandı. Tüm veya belirli bir anlaşmalar türü için verileri çözümlemek istiyorsanız filtreyi kaldırın.
...
- **Anlaşma türü**: varsayılan olarak **ortak satış** ve **iş ortağı** taraflıdır, ancak seçiminize göre verilerin tümünü veya özel veya iş ortağı olarak ele aldığı anlaşmaları seçebilirsiniz.
...
Başvuruyu dışa aktarabilir ve kayıt verilerini dağıtabilirsiniz. Aşağıdaki bilgiler dışa aktarma işlevselliğini açıklamaktadır.

- **Dışarı aktar** düğmesine tıklayarak **en fazla 5000 kaydı** dışarı aktarabilirsiniz. Kayıtlar, başvuru oluşturma tarihine göre azalan sırada sıralanır.
- Dışa aktarma işlevi, zaman çizelgesini ve uygulanan filtreleri hesaba alır.
- İş ortağı, raporu CSV (virgülle ayrılmış değer) ya da TSV (sekmeyle ayrılmış değer) biçiminde indirmeyi tercih edebilir.
- Kayıtların indirilmesi birkaç dakika sürebilir.
- İndirmenin tamamlanmasını beklemeniz gerekir. Dışarıda gezinmek dışarı aktarma işlemini iptal eder.
...
> [!NOTE]
> **Referanslar** içgörü sayfasında yalnızca İş Ortağı Merkezi'nde oluşturulan tavsiyelere ait veriler gösteriliyor. [ortak satış Bağlan](psc-to-pc.md) veya diğer mekanizmalarla oluşturulan başvuruların verilerini göstermez.

> [!TIP]
> [Bir çözüm sağlayıcısı bulma](https://www.microsoft.com/solution-providers/home) deneyiminde iş profilinizin nasıl çalıştığını görmek Için, [müşteri adayları Öngörüler sayfasını](referral-leads-insights.md)gözden geçirin.
