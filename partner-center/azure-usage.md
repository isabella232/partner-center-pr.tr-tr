...
description: Bir sanal makineyi (VM) müşterinizin bilgi işlem ihtiyaçlarına göre boyutlama hakkında bilgi Microsoft Azure satın alın.
ms.topic: how-to
...
Bu makalede, bir sanal makineyi (VM) müşteriler için rezervasyon satın alan müşterilerin bilgi işlem ihtiyaçlarına göre Microsoft Azure açıklanmıştır.
...
Müşterileriniz adına Microsoft Azure rezervasyon satın alırken, müşterinin bilgi işlem ihtiyaçlarını karşılamak için boyutlandırıldı bir sanal makine (VM) seçmeniz gerekir. Bu bilgileri şu yöntemlerden birini kullanarak bulabilirsiniz:
...
Bu yöntemlerin her birini kullanmaya ilişkin yönergeler aşağıda verilmiştir. Rezervasyon satın aldıktan sonra rezervasyon indirimi, rezervasyonun öznitelikleri ve miktarıyla eşleşen sanal makinelere otomatik olarak uygulanır. Rezervasyonu bir VM'ye atamanız gerek değildir.
...
Müşteri adına bir Azure Ayrılmış VM Örneği satın aldıktan sonra, VM alanı için önceden ödemeye uygulanan indirim, müşterinin rezervasyonun öznitelikleriyle ve miktarıyla eşan sanal makinelere otomatik olarak uygulanır.

Aşağıdaki yöntemlerden birini kullanarak müşterinin rezervasyon kullanımını doğrular ve rezervasyon indirimlerinin hangi sanal makinelere uygulandığını görebilirsiniz:
...
>[!IMPORTANT]
>Microsoft Windows Server gibi yazılım maliyetleri şu anda VM rezervasyonu fiyatına dahil değildir ve sipariş kaydında ve faturada ayrı satır öğeleri olarak görünür. Ancak, bir müşterinin Azure Hibrit Kullanım Avantajı varsa yazılım maliyetleri uygulanmaz. Daha fazla bilgi için [bkz. ayrılmış Windows dahil değil yazılım maliyetleri.](/azure/billing/billing-reserved-instance-windows-software-costs)  
...
|Azure'da müşterileriniz için Azure rezervasyonları İş Ortağı Merkezi   | [Azure rezervasyonları satın alma](azure-reservations-buying.md)
...
|İş Ortağı Merkezi API'sini kullanarak Azure rezervasyonları satın alma | [İş Ortağı Merkezi geliştirici belgelerinde](/partner-center/develop/purchase-azure-reservations) Azure Ayrılmış VM Örnekleri satın alma   |
