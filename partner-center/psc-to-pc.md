...
Bu makalede, iş ortaklarının iş ortağı satış anlaşmaları oluşturma ve yönetmeye devam etmek için İş Ortağı Satışları Bağlan'den İş Ortağı Merkezi'a (PC) İş Ortağı Merkezi.
...
- İş Ortağı Merkezi'de oturum a almak için bir [iş e-postası İş Ortağı Merkezi.](https://partner.microsoft.com/)
- Hesap yöneticisinin yardımıyla İş Ortağı Merkezi [ayarlayın.](permissions-overview.md)
- Bu belgeyi okuyarak İş Ortağı Merkezi ortak satış yapmayı öğrenin.
...
>[!IMPORTANT]
> PSC başlığında Microsoft İş Ortağı Ağı (MPN) kimliğinin, İş Ortağı Merkezi'daki MPN konumları listesinde kullanılabilir olduğundan emin İş Ortağı Merkezi.
...
- Panoda oturum alasınız için bir iş İş Ortağı Merkezi [gerekir.](https://partner.microsoft.com/dashboard)
...
- PSC'de oturum a İş Ortağı Merkezi hesap ne olursa olsun, kullanıcı hesabı ayarlama işleminin tamamlandıktan sonra PSC yöneticinize sorun.
...
>[!Important]
>- Rolünüz yalnızca PSC'de kullanıcıları yönetmeyi içeriyorsa, şirketinizin hesap [](permissions-overview.md#manage-mpn-membership-and-your-company) yöneticisinin size şirket içinde hesap yöneticisi rolünü atamasını İş Ortağı Merkezi. 
...
Hesap yöneticileri, referans kılavuzunun yanındaki ortak satış fırsatları sayfasında bir PSC kullanıcı geçiş sihirbazı bağlantısı görebilir. Bağlantıyı seçerek kullanıcı geçişini başlatabilirsiniz. Yöneticiler, kullanıcı geçişini başlatmak için bağlantıyı seçerek. Bu kullanıcı geçişi adımını, tüm kullanıcılara bir rolde uygun roller atanana kadar birden çok İş Ortağı Merkezi.
...
- PC AAD Kiracısı - Microsoft Azure Active Directory 'de kullanıcıların atandığı Microsoft Azure Active Directory (Azure AD) İş Ortağı Merkezi
- Durum - Geçişin durumu için üç olası durum vardır
    - **Geçirilmez** - Kullanıcıya herhangi bir referans İş Ortağı Merkezi rolü atanmamış
...
2. PSC kullanıcısı, etki alanındaki hesaplardan farklı bir hesap kullanıyor olabilir İş Ortağı Merkezi.
   ...
   - Kullanıcı listesinin üst kısmında Filtre  açılan menüsünü seçin ve seçeneğini Genel yönetici **olarak belirleyin.** Ardından sayfada ilgili e-posta adresleriyle birlikte tüm genel yöneticiler görüntülenir. İş hesabınız için referans yöneticisi rolünü atamasını istemek için bunlardan birini sorun.
      ...
      Genel yönetici, Azure AD kiracınız içinde yeni bir kullanıcı hesabı oluşturabilir veya diğer etki alanı hesabı kullanıcılarına konuk kullanıcı erişimi atayabilirsiniz. Tüm PSC anlaşma yöneticileri ve kullanıcıları için hesaplar ayarlanmaz, İş Ortağı Merkezi'de oturum açmaları, sol gezinti menüsünden Referanslar'ı seçmeleri ve Referanslar sayfasını görene kadar onayları gerekir. 
...
PsC'den İş Ortağı Merkezi'a geçiş için yalnızca bazı İş Ortağı Merkezi. Bu geçiş sihirbazı, iş ortaklarının anlaşmalarını İş Ortağı Merkezi müşterileri ile etkin bir şekilde çalışarak anlaşmaları kapatmalarına yardımcı olmak için oluşturulur. **Yalnızca geçerli iş ortağı hesabı ayrıntıları (geçerli MPN ID) ile 1 Ocak 2020 ' den oluşturulan ve açık durumda olan anlaşmalar geçiş için uygun değildir.**
...
|İş Ortağı Merkezi'de kazandınız anlaşmaları İş Ortağı Merkezi |[Yeni bir anlaşma kaydetme](./register-deals.md)
...
- [İş Ortağı Merkezi ortak satış](https://partner.microsoft.com/resources/detail/co-sell-operating-model-guide-pptx) çalışma kılavuzu - müşteri adaylarını veya ortak satış fırsatlarını yönetmek ve anlaşmaları kaydetmek İş Ortağı Merkezi aracılığıyla bir operasyon modelini belirlemeye yönelik kılavuz.
