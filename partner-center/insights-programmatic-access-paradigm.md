...
1. İstemci Uygulaması, Rapor Sorgusu OLUŞTURMA API'sini çağırarak özel rapor [şemasını/şablonunu tanımlayabilir.](#create-report-query-api) Alternatif olarak, burada listelenen rapor şablonu kitaplık örneklerinden bir rapor şablonu (QueryId) [de kullanabilirsiniz.](insights-programmatic-system-queries.md)
...
API, sütunların ve ölçümlerin dışarı aktarması gereken veri kümesini tanımlayan özel sorgular oluşturmanıza yardımcı olur. API, iş ihtiyaçlarınıza göre yeni bir raporlama şablonu oluşturma esnekliği sağlar.  

Ayrıca, bizim [sağmız olan sistem sorgularını](insights-programmatic-system-queries.md) da kullanabilirsiniz. Özel rapor şablonları gerekli değilken, sağlanan sistem sorgularının QueryId'lerini kullanarak rapor [oluşturma API'sini](#create-report-api) doğrudan çağırabilirsiniz.  
...
|    QueryStartTime     |    No     |    İsteğe bağlı olarak, verileri ayıklayan sorgunun başlangıç saatini belirtir. Bu parametre yalnızca, true olarak ayarlanmış bir kerelik yürütme raporu için geçerlidir `ExecuteNow` . Sorguda verilen bu parametre geçersiz kılmaları ayarlanıyor `TIMESPAN` . Biçim yyyy-MM-ddTHH: mm: ssZ olmalıdır     |    Dize olarak zaman damgası     |
...
|    ModifiedTime     |    UTC Raporun son değiştirilme zamanı: yyyy-MM-ddTHH:mm:ssZ     |
...
|    message     |    Olası değerler 200, 400, 401, 403, 500'tir. API'nin yürütülmesinden gelen durum iletisi     |
|        |        |
...
|    executionStatus    |    No    |    Dize/enum    |    Yalnızca bu bağımsız değişkende verilen executionStatus değerine sahip raporların ayrıntılarını almak için filtreyi seçin. <br> Geçerli değerler: `Pending` , `Running` ve `Paused` `Completed` . <br> `Completed` varsayılan değerdir. <br> Birden çok durum, noktalı virgül ";" ile ayrılarak belirtilebilir.    |
