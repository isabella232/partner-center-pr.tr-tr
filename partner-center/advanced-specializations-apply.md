---
title: Gelişmiş uzmanlıklar için başvurma
ms.topic: how-to
ms.date: 09/27/2021
ms.service: partner-dashboard
ms.subservice: partnercenter-membership
description: Microsoft iş ortağı dizininde iş profilinizi güçlendirin. Gelişmiş uzmanlıklar için İş Ortağı Merkezi ve kazanmak için İş Ortağı Merkezi kullanmayı öğrenin.
...
Bu makalede gelişmiş bir uzmanlık için İş Ortağı Merkezi, ilerlemenizi kontrol etmek [ve](advanced-specializations.md)gerekli adımları ve doğrulamaları tamamlamak için İş Ortağı Merkezi panosunun nasıl kullanımı açıklanmıştır. Ayrıca, denetim veya müşteri İş Ortağı Merkezi gerektiren bazı gelişmiş uzmanlıkları elde etmek için bu özellikleri kullanma örnekleri de göreceğiz.
...
Sürecin büyük bir İş Ortağı Merkezi. Ancak bu alana yalnızca belirli bir [](permissions-overview.md) İş Ortağı Merkezi rolüne sahip kullanıcılar İş Ortağı Merkezi erişebilirsiniz.
...
1. İş Ortağı Merkezi oturum [açın](https://partner.microsoft.com/dashboard/home) ve Üyelik **kutucuğunu** seçin.

2. Gelişmiş **uzmanlıklar'ı** ve ardından ilgilendiğiniz alanı seçin: Azure, Modern çalışma, Güvenlik veya İş uygulamaları.
...
3. Ayrıntıları görüntülemek için gelişmiş özelleştirmenin adını seçin. Belirli gereksinimleri ve adımları, geçerli durumunuz ve diğer adımlara veya kaynaklara yönelik bağlantıların olduğunu göreceğiz.
   ...
   Herhangi bir gelişmiş uzmanlık elde etme İş Ortağı Merkezi durumunu görmek için bu bölgeye geri dönabilirsiniz.

4. Gelişmiş uzmanlık için tüm gereksinimleri tamamlandıktan sonra, belirli bir gelişmiş İş Ortağı Merkezi için durumunuz  Etkin olarak değiştirildi mi görmek için bu alanı yeniden kontrol edin.
...
## <a name="use-partner-center-to-schedule-an-audit-or-include-customer-references-for-advanced-specializations"></a>Denetim İş Ortağı Merkezi veya gelişmiş uzmanlıklara yönelik müşteri başvurularını dahil etmek için İş Ortağı Merkezi'i kullanın
...
- Üçüncü taraf denetçi ile denetim zamanlama
- Müşteri başvurularını sağlama

Her iki görev de aynı İş Ortağı Merkezi.

Üçüncü taraf denetçi ile denetim gerektiren gelişmiş uzmanlıklar şunlardır:
...
Müşteri başvuruları gerektiren gelişmiş uzmanlıklar şunlardır:

- Benimseme ve Değişiklik Yönetimi
- Microsoft Teams çağrısı
- Toplantılar ve toplantı odaları Microsoft Teams
...
Denetim veya müşteri başvurusu gerektirmeyen gelişmiş uzmanlıklar şunlardır:
...
Kendi önkoşullarının yanı sıra, bazı gelişmiş uzmanlıklar da zamanlanmış bir denetimden *geçmenizi gerektirir.* Denetim gerektiren gelişmiş uzmanlıkların önceki listesine bakın.

Ardından, aşağıdaki adımları takip edin ve aşağıdaki adımları İş Ortağı Merkezi:
...
3. Bir denetim zamanlamanız gereken gelişmiş özelleştirmenin adını seçin. Örneğin, Web Uygulamalarını Modernleştirme'yi **seçerek öğesini Microsoft Azure.** Daha sonra bu gelişmiş uzmanlık için önkoşullar sayfası görüntülenir.
   ...
   :::image type="content" source="images/advanced-specializations/as-1.png" alt-text="Belirli bir gelişmiş uzmanlık kazanma gereksinimlerini gösteren örnek önkoşullar sayfası.":::

4. Bu gelişmiş uzmanlık için tüm önkoşulları karşılarken bu sayfadan **Denetim zamanlama** düğmesini seçebilirsiniz.

   - Bu sayfada gösterilen tüm önkoşulları karşılana kadar durumunuz Başlamadı **olarak kalır.** Denetim **zamanlama düğmesi** de devre dışı kalır. 
   - Denetimle birlikte tüm önkoşullar karşı yapıldıktan sonra durumunuz Etkin olarak **değişir.**

#### <a name="current-view"></a>[Geçerli görünüm](#tab/current-view)

1. [İş Ortağı Merkezi](https://partner.microsoft.com/dashboard/home) panosunda oturum açın ve **menüden MPN'yi** ve ardından **Adv.** Uzmanlıklar'ı seçerek kullanılabilir tüm gelişmiş özelleştirmeleri görebilirsiniz.

2. Bir denetim zamanlamanız gereken gelişmiş özelleştirmenin adını seçin. Örneğin, Web Uygulamalarını Modernleştirme'yi **seçerek öğesini Microsoft Azure.** Daha sonra bu gelişmiş uzmanlık için önkoşullar sayfası görüntülenir.

   :::image type="content" source="images/advanced-specializations/as-1.png" alt-text="Belirli bir gelişmiş uzmanlık kazanma gereksinimlerini gösteren örnek önkoşullar sayfası.":::

3. Bu gelişmiş uzmanlık için tüm önkoşulları karşılarken bu sayfadan **Denetim zamanlama** düğmesini seçebilirsiniz.
...
## <a name="provide-customer-references-in-partner-center"></a>Verilerde müşteri başvurularını İş Ortağı Merkezi

Kendi önkoşullarının yanı sıra, bazı gelişmiş uzmanlıklar da müşteri *başvurularını sağlamanızı gerektirir.* Müşteri başvuruları gerektiren gelişmiş uzmanlıkların önceki listesine bakın.

Ardından, aşağıdaki adımları takip edin ve aşağıdaki adımlarda müşteri İş Ortağı Merkezi:
...
3. Müşteri başvurularını sağlamanız gereken gelişmiş özelleştirmenin adını seçin. Örneğin, Microsoft Teams için **Toplantılar ve Toplantı Odaları'Microsoft Teams:**
   ...
   Bu gelişmiş uzmanlık için önkoşullar sayfası görüntülenir. Gold uzmanlığı, sertifikaları ve performans gereksinimine ek olarak, bu gelişmiş uzmanlık, dağıtma ve yönetme becerinizi gösteren üç müşteri başvurusu sağlamanızı gerektirir.

4. Bu sayfada gösterilen tüm önkoşulları karşılar ve müşteri başvurularını sağlayana kadar durumunuz Başlamadı **olarak kalır.** Tüm önkoşullar karşı ardından durumunuz Etkin olarak **değişir.**
...
3. Müşteri başvurularını sağlamanız gereken gelişmiş özelleştirmenin adını seçin. Örneğin, Microsoft Teams için Toplantılar **ve Toplantı Odaları'Microsoft Teams.**
   ...
   :::image type="content" source="images/advanced-specializations/as-2.png" alt-text="Müşteri başvuruları gerektiren örnek gelişmiş uzmanlık önkoşulları sayfası.":::

4. Bu sayfada gösterilen tüm önkoşulları karşılar ve müşteri başvurularını sağlayana kadar durumunuz Başlamadı **olarak kalır.** Tüm önkoşullar karşı ardından durumunuz Etkin olarak **değişir.**

* * *

## <a name="how-azure-consumption-revenue-acr-is-calculated-for-each-advanced-specialization"></a>Her gelişmiş uzmanlık için Azure Tüketim Geliri (ACR) nasıl hesaplanır?

- İş ortağı-müşteri ilişkilendirmeleri [dahil:](https://support.microsoft.com/topic/link-a-partner-id-for-azure-performance-pal-or-dpor-a8eed43b-82a8-f017-3b1a-f9c8aa385d32) Bir müşteriyle Kaydın Dijital İş Ortağı ilişkilendirmesi ya da bir müşteri için CSP doğrudan fatura iş ortağı veya CSP dolaylı kurumsal bayi ya da bir müşteriyle İş Ortağı Yönetici Bağlantısı [(PAL)](/azure/cost-management-billing/manage/link-partner-id) ilişkilendirmesi.
- Deneme, Avantajlar programları, Destek ve Dahili Teklif türlerine sahip aboneliklerden gelen ACR, gelişmiş uzmanlık için sayılmaz.

### <a name="azure-consumption-revenue-fields"></a>Azure Tüketim geliri alanları
...
- Microsoft Azure'a Windows Server ve SQL Server Geçişi
    - Windows ACR: 
        ...
        - İşlem OS = WINDOWS THEN compute OS Özniteliği = WINDOWS, UNKNOWN veya Compute OS = LINUX THEN Compute OS Özniteliği = WINDOWS-AHUB
...
- Linux ve Open-Source Veritabanı'Microsoft Azure
    - Linux Sanal Makineleri (VM)ACR:
        - 1. Seçenek:
            ...
            - Hizmet Düzeyi 4= Cloud Services MS Serisi, Sanal Makineler MS Serisi, Sanal Makineler MS Serisi Windows, Sanal Makineler MSv2 Serisi, Sanal Makineler MSv2 Serisi Windows, MS Serisi Ayrılmış Ana Bilgisayarı, MSv2 Serisi Ayrılmış Konak dışındaki tüm hizmetler
        - 2. Seçenek: Yalnızca Hizmet Düzeyi 4'te filtre gerektirir; İşlem OS/İşlem OS Öznitelikleri gibi diğer alanlar gerekli değildir
            - Hizmet Düzeyi 4 = Red Hat Enterprise Linux, HA ile Red Hat Enterprise Linux, SUSE Linux Enterprise Server Basic, SUSE Linux Enterprise Server for HPC Priority, SUSE Linux Enterprise Server for HPC Standard, SUSE Linux Enterprise Server Priority, SUSE Linux Enterprise Sunucu Standardı
...
- Bulut Güvenliği
    - Karma Ortam XDR ve Ağ Güvenliği ACR: SQL için Hizmet Düzeyi 4 = Azure Defender, Azure Defender Depolama, Kubernetes için Azure Defender, Azure Defender for Depolama, Application Gateway WAF v2, WAF Application Gateway, Azure Active Directory B2C, Azure Active Directory Domain Services, Azure Active Directory Kimlikler, Azure Bastion, Azure DDOS Koruması, Azure Güvenlik Duvarı, Azure Güvenlik Duvarı Yöneticisi, Azure Front Door Service, IoT için Azure Defender, Azure Ayrılmış HSM, Key Vault, Ağ Azure Defender, App Service için Azure Defender, Sentinel
    
- Microsoft Azure’da SAP
    - SAP workloads ACR: Service Level 4 = Cloud Services MS Series, Virtual Machines MS Series, Virtual Machines MS Series Windows, Virtual Machines MSv2 Series, Virtual Machines MSv2 Series Windows, MS Series Dedicated Host, MSv2 Series Dedicated Host, Azure üzerinde SAP HANA Büyük Örnekleri,SAP Cloud Platform Alert Notification, SAP Cloud Platform Extension Factory - Kyma Runtime,SAP Cloud Platform Integration Suite - Ek İletiler,  SAP Cloud Platform Integration Suite - Standard Sürümü, SAP Cloud Platform Transport Management, SAP Edge Services, SAP Embrace API Management,SAP Embrace Application Logging, SAP Embrace Application Runtime, SAP Embrace Bandwidth, SAP Embrace Business Application Studio, SAP Embrace Business Rules, SAP Embrace Cloud Integration, SAP Embrace Credential Store, SAP Embrace Custom Domain, SAP Embrace Data Intelligence, SAP Embrace Enterprise Mesajlaşma, SAP Embrace Extension Factory, sunucusuz çalışma zamanı, SAP Embrace Hana Cloud, SAP Embrace Identity Authentication, SAP Embrace Job Scheduler, SAP Embrace MACC, SAP Embrace Mobile Services,SAP Embrace Object Store Service, SAP Embrace Open Connectors, SAP Embrace Portal, SAP Embrace Process Visibility, SAP Embrace Web Analytics, SAP Embrace workflow, SAP HANA Service, SAP Web IDE,  BareMetal Infrastructure, Virtual Machines MdSv2 Series, Virtual Machines MdSv2 Series Windows
